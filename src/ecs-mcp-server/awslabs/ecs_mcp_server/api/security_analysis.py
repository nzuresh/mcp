...
class SecurityAnalyzer:
                    ...
                    recommendations.append(
                        {
                            ...
                            "recommendation": "Enable transit encryption for EFS volumes to protect data in transit",
                        }
                    )
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": "iam_security",
                            "resource": f"EFS Volume: {volume_name}",
                            "issue": "EFS volume not using access points, missing fine-grained access control",
                            "recommendation": "Configure EFS access points to enforce POSIX permissions and user/group mappings",
                        }
                    )
                    ...
                    recommendations.append(
                        {
                            ...
                            "issue": "FSx volume credentials not managed through AWS Secrets Manager",
                            "recommendation": "Store FSx credentials in AWS Secrets Manager for secure access",
                        }
                    )
                ...
                recommendations.append(
                    {
                        ...
                        "issue": "Container uses outdated base image with known security vulnerabilities",
                        "recommendation": "Update to supported base image versions with current security patches",
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "category": "container_security",
                        "resource": f"Container: {container_name}",
                        "issue": "Container lacks init process, potentially causing zombie processes and signal handling issues",
                        "recommendation": "Enable initProcessEnabled to properly handle signals and prevent zombie processes",
                    }
                )
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": "container_security",
                            "resource": f"Container: {container_name}",
                            "issue": f"Tmpfs mount size {size} bytes is very large and could lead to memory exhaustion",
                            "recommendation": "Limit tmpfs mount sizes to prevent memory-based DoS attacks",
                        }
                    )
                ...
                recommendations.append(
                    {
                        ...
                        "category": "container_security",
                        "resource": f"Container: {container_name}",
                        "issue": f"Shared memory size {shared_memory_size} bytes is large and could be exploited",
                        "recommendation": "Limit shared memory size to minimum required for application functionality",
                    }
                )
                            ...
                            recommendations.append(
                                {
                                    ...
                                    "category": "secrets",
                                    "resource": f"Container: {container_name}",
                                    "issue": f"Secret {secret_name} uses Parameter Store instead of Secrets Manager for sensitive data",
                                    "recommendation": "Use AWS Secrets Manager for passwords, API keys, and credentials for automatic rotation and enhanced security",
                                }
                            )
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container: {container_name}",
                    "issue": "Using latest tag in base images creates unpredictable security posture",
                    "recommendation": "Use specific version tags for base images to ensure consistent security updates",
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container: {container_name}",
                    "issue": "Container may be using full-featured base image with larger attack surface",
                    "recommendation": "Consider using minimal base images (Alpine, distroless) to reduce attack surface",
                }
            )
                    ...
                    recommendations.append(
                        {
                            ...
                            "recommendation": (
                                "Remove unnecessary capabilities and follow principle of least privilege"
                            ),
                        }
                    )
                ...
                recommendations.append(
                    {
                        ...
                        "category": "runtime_security",
                        "resource": f"Container: {container_name}",
                        "issue": "Container lacks seccomp profile, allowing unrestricted system calls that could be exploited by attackers",
                        "recommendation": "Configure a seccomp profile to restrict system calls and reduce attack surface",
                        "implementation_steps": [
                            "Create a custom seccomp profile or use Docker's default profile",
                            "Add seccompProfile configuration to linuxParameters in task definition",
                            "Test application functionality with the profile enabled",
                            "Monitor for any blocked system calls and adjust profile as needed",
                        ],
                        "aws_cli_example": 'aws ecs register-task-definition --cli-input-json \'{"family":"my-task","containerDefinitions":[{"linuxParameters":{"seccompProfile":"default"}}]}\'',
                        "compliance_frameworks": [
                ...
                recommendations.append(
                    {
                        ...
                        "category": "runtime_security",
                        "resource": f"Container: {container_name}",
                        "issue": "Container lacks AppArmor profile, missing mandatory access control protection",
                        "recommendation": "Configure AppArmor profile for mandatory access control and additional security layer",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'aws ecs register-task-definition --cli-input-json \'{"family":"my-task","containerDefinitions":[{"linuxParameters":{"apparmorProfile":"docker-default"}}]}\'',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "runtime_security",
                        "resource": f"Container: {container_name}",
                        "issue": "Container can gain new privileges during execution, enabling privilege escalation attacks",
                        "recommendation": "Enable noNewPrivileges flag to prevent privilege escalation within the container",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'aws ecs register-task-definition --cli-input-json \'{"family":"my-task","containerDefinitions":[{"linuxParameters":{"noNewPrivileges":true}}]}\'',
                        "compliance_frameworks": [
            ...
            recommendations.append(
                {
                    ...
                    "category": "runtime_security",
                    "resource": f"Container: {container_name}",
                    "issue": "Container lacks Linux security parameters, missing critical runtime security controls",
                    "recommendation": "Configure linuxParameters with seccomp, AppArmor, and noNewPrivileges for comprehensive runtime security",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": 'aws ecs register-task-definition --cli-input-json \'{"family":"my-task","containerDefinitions":[{"linuxParameters":{"seccompProfile":"default","noNewPrivileges":true}}]}\'',
                    "compliance_frameworks": [
                        "CIS Docker Benchmark",
                        "NIST Cybersecurity Framework",
                    ],
                        ...
                        if repositories:
                                ...
                                recommendations.append(
                                    {
                                        ...
                                        "issue": "ECR repository does not have vulnerability scanning enabled, missing critical security vulnerability detection",
                                        "recommendation": "Enable scan on push to automatically scan container images for known vulnerabilities",
                                        "implementation_steps": [
                                            "Enable scan on push in ECR repository settings",
                                            "Configure scanning for existing images",
                                            "Set up CloudWatch alarms for high/critical vulnerabilities",
                                            "Implement CI/CD pipeline checks to block vulnerable images",
                                        ],
                                        "aws_cli_example": f"aws ecr put-image-scanning-configuration --repository-name {repo_name} --image-scanning-configuration scanOnPush=true",
                                        "compliance_frameworks": [
                                            "NIST Cybersecurity Framework",
                                            "CIS Controls",
                                        ],
                            ...
                            try:
                                ...
                                if critical_count > 0:
                                    recommendations.append(
                                        {
                                            "title": "Address Critical Vulnerabilities in Container Image",
                                            "severity": "Critical",
                                            "category": "image_security",
                                            "resource": f"Container Image: {image}",
                                            "issue": f"Container image has {critical_count} critical vulnerabilities that pose immediate security risks",
                                            "recommendation": "Immediately update base image or packages to address critical vulnerabilities before deployment",
                                            "implementation_steps": [
                                                "Review detailed vulnerability findings in ECR console",
                                            ...
                                            "aws_cli_example": f"aws ecr describe-image-scan-findings --repository-name {repo_name} --image-id imageTag={image_tag}",
                                            "compliance_frameworks": [
                                                "OWASP Top 10",
                                                "NIST Cybersecurity Framework",
                                            ],
                                ...
                                if high_count > 0:
                                    recommendations.append(
                                        {
                                            "title": "Address High Severity Vulnerabilities in Container Image",
                                            "severity": "High",
                                            "category": "image_security",
                                            "resource": f"Container Image: {image}",
                                            "issue": f"Container image has {high_count} high severity vulnerabilities that should be addressed promptly",
                                            "recommendation": "Update container image to address high severity vulnerabilities within next maintenance window",
                                            "implementation_steps": [
                                            ...
                                            "aws_cli_example": f'aws ecr describe-image-scan-findings --repository-name {repo_name} --image-id imageTag={image_tag} --filter \'{{"severity":["HIGH"]}}\'',
                                            "compliance_frameworks": [
                                                "NIST Cybersecurity Framework",
                                                "ISO 27001",
                                            ],
                                    ...
                                    recommendations.append(
                                        {
                                            ...
                                            "category": "image_security",
                                            "resource": f"Container Image: {image}",
                                            "issue": f"Container image has {medium_count} medium severity vulnerabilities that should be reviewed",
                                            "recommendation": "Review and plan remediation for medium severity vulnerabilities during regular maintenance",
                                            "implementation_steps": [
                                            ...
                                            "aws_cli_example": f'aws ecr describe-image-scan-findings --repository-name {repo_name} --image-id imageTag={image_tag} --filter \'{{"severity":["MEDIUM"]}}\'',
                                ...
                                recommendations.append(
                                    {
                                        ...
                                        "category": "image_security",
                                        "resource": f"Container Image: {image}",
                                        "issue": "Container image has not been scanned for vulnerabilities",
                                        "recommendation": "Initiate vulnerability scan to identify potential security issues",
                                        "implementation_steps": [
                                        ...
                                        "aws_cli_example": f"aws ecr start-image-scan --repository-name {repo_name} --image-id imageTag={image_tag}",
                                        "compliance_frameworks": ["Security Best Practices"],
                                    }
                                )
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container: {container_name}",
                    "issue": "Container uses external registry without integrated vulnerability scanning",
                    "recommendation": "Migrate to Amazon ECR for integrated vulnerability scanning and better security controls",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": f"aws ecr create-repository --repository-name {container_name.lower()}",
                ...
                recommendations.append(
                    {
                        ...
                        "resource": f"Service: {service_name}",
                        "issue": "Service Connect enabled without proper namespace configuration",
                        "recommendation": "Configure Service Connect namespace for proper service discovery and security boundaries",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f"aws servicediscovery create-private-dns-namespace --name {cluster_name}-mesh --vpc vpc-xxxxxx",
                        "compliance_frameworks": [
                        ...
                        recommendations.append(
                            {
                                ...
                                "category": "service_mesh",
                                "resource": f"Service: {service_name}",
                                "issue": f"Service Connect communication on port {port} may not be encrypted",
                                "recommendation": "Configure TLS encryption for service-to-service communication",
                                "implementation_steps": [
            ...
            recommendations.append(
                {
                    ...
                    "category": "service_mesh",
                    "resource": f"Service: {service_name}",
                    "issue": "Service lacks service mesh capabilities for secure service-to-service communication",
                    "recommendation": "Consider enabling ECS Service Connect for encrypted service-to-service communication and observability",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": f"aws ecs update-service --cluster {cluster_name} --service {service_name} --service-connect-configuration enabled=true",
                ...
                recommendations.append(
                    {
                        ...
                        "recommendation": "Enable TLS encryption in App Mesh for secure service-to-service communication",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'aws appmesh update-virtual-service --mesh-name my-mesh --virtual-service-name my-service --spec \'{"provider":{"virtualRouter":{"virtualRouterName":"my-router"}}}\'',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "service_mesh",
                        "resource": f"Service: {service_name}",
                        "issue": "App Mesh has TLS but lacks mutual TLS authentication for enhanced security",
                        "recommendation": "Implement mutual TLS (mTLS) to ensure both client and server authentication in service mesh",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'aws appmesh update-virtual-service --mesh-name my-mesh --virtual-service-name my-service --spec \'{"provider":{"virtualNode":{"virtualNodeName":"my-node"}}}\'',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "network_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Service lacks security group configuration for network access control",
                        "recommendation": "Configure security groups to implement network-level access controls and micro-segmentation",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f'aws ec2 create-security-group --group-name {service_name}-sg --description "Security group for {service_name}"',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "network_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Security group rules should be reviewed for least privilege access",
                        "recommendation": "Regularly review and audit security group rules to ensure minimal required access",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f"aws ec2 describe-security-groups --group-ids {' '.join(security_groups)}",
                    ...
                    recommendations.append(
                        {
                            ...
                            "resource": f"Service: {service_name}",
                            "issue": "Service discovery DNS configuration should be secured against DNS attacks",
                            "recommendation": "Implement DNS security best practices for service discovery",
                            "implementation_steps": [
                            ...
                            "aws_cli_example": f"aws servicediscovery get-service --id {registry_arn.split('/')[-1]}",
            ...
            recommendations.append(
                {
                    ...
                    "recommendation": "Implement DNS security monitoring to detect and prevent DNS-based attacks",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": "aws route53resolver create-resolver-query-log-config --name dns-security-logs",
                ...
                recommendations.append(
                    {
                        ...
                        "resource": f"Service: {service_name}",
                        "issue": "Service may communicate with AWS services over the internet instead of private VPC endpoints",
                        "recommendation": "Configure VPC endpoints for AWS services to keep traffic within AWS network",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f"aws ec2 create-vpc-endpoint --vpc-id vpc-xxxxx --service-name com.amazonaws.{region}.ecr.dkr",
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container Image: {image}",
                    "issue": "Container image lacks cryptographic signature verification, allowing potential supply chain attacks",
                    "recommendation": "Implement image signing using AWS Signer or Docker Content Trust to ensure image integrity and authenticity",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": "aws signer put-signing-profile --profile-name container-signing --signing-material certificateArn=arn:aws:acm:region:account:certificate/cert-id",
                    "compliance_frameworks": [
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container Image: {image}",
                    "issue": "Container image lacks provenance metadata, making supply chain verification difficult",
                    "recommendation": "Implement image provenance tracking to maintain build and supply chain transparency",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": "# Use cosign or similar tool: cosign sign --key cosign.key image-uri",
                    "compliance_frameworks": [
                        "SLSA Framework",
                        "Supply Chain Security",
                        "NIST SSDF",
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container Image: {image}",
                    "issue": "Container image lacks comprehensive supply chain security verification",
                    "recommendation": "Implement end-to-end supply chain security controls for container images",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": f"aws ecr start-image-scan --repository-name {image.split('/')[-1].split(':')[0]} --image-id imageTag=latest",
                    "compliance_frameworks": [
                        "Supply Chain Security",
                        "NIST SSDF",
                        "SLSA Framework",
        ...
        recommendations.append(
            {
                ...
                "recommendation": "Implement comprehensive package vulnerability scanning for all container dependencies",
                "implementation_steps": [
                ...
                "aws_cli_example": f"aws ecr describe-image-scan-findings --repository-name {image.split('/')[-1].split(':')[0]}",
        ...
        recommendations.append(
            {
                ...
                "category": "image_security",
                "resource": f"Container Image: {image}",
                "issue": "Cannot verify if image follows secure multi-stage build practices to minimize attack surface",
                "recommendation": "Ensure container images use multi-stage builds to exclude build tools and reduce attack surface",
                "implementation_steps": [
        ...
        recommendations.append(
            {
                ...
                "category": "image_security",
                "resource": f"Container Image: {image}",
                "issue": "Base image may contain known vulnerabilities that affect container security",
                "recommendation": "Regularly analyze and update base images to address known vulnerabilities",
                "implementation_steps": [
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container Image: {image}",
                    "issue": "Container uses full OS base image which increases attack surface and potential vulnerabilities",
                    "recommendation": "Consider using minimal base images like distroless or alpine to reduce attack surface",
                    "implementation_steps": [
            ...
            recommendations.append(
                {
                    ...
                    "category": "image_security",
                    "resource": f"Container Image: {image}",
                    "issue": "Container image uses mutable tag, allowing potential image substitution attacks",
                    "recommendation": "Use immutable image references with SHA256 digest to ensure image integrity",
                    "implementation_steps": [
                ...
                recommendations.append(
                    {
                        ...
                        "resource": f"Cluster: {cluster_name}",
                        "issue": "ECS Exec is enabled without KMS encryption, potentially exposing sensitive command data",
                        "recommendation": "Configure customer-managed KMS key for ECS Exec session encryption",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f'aws ecs put-cluster-capacity-providers --cluster {cluster_name} --configuration executeCommandConfiguration="{{kmsKeyId=arn:aws:kms:{region}:account:key/key-id}}"',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "ecs_advanced",
                        "resource": f"Cluster: {cluster_name}",
                        "issue": "ECS Exec sessions are not being logged, missing audit trail for security compliance",
                        ...
                        "aws_cli_example": f'aws ecs put-cluster-capacity-providers --cluster {cluster_name} --configuration executeCommandConfiguration="{{logging=OVERRIDE,logConfiguration={{cloudWatchLogGroupName=/aws/ecs/exec/{cluster_name}}}}}"',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "ecs_advanced",
                        "resource": f"Service: {service_name}",
                        "issue": "Blue/Green deployment lacks circuit breaker protection against failed deployments",
                        "recommendation": "Enable deployment circuit breaker to automatically rollback failed Blue/Green deployments",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f'aws ecs update-service --cluster {cluster_name} --service {service_name} --deployment-configuration deploymentCircuitBreaker="{{enable=true,rollback=true}}"',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "ecs_advanced",
                        "resource": f"Service: {service_name}",
                        "issue": "Service uses Spot instances which may have security implications for stateful workloads",
                        "recommendation": "Ensure Spot instance usage is appropriate for workload security requirements",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": "# Review capacity provider strategy and workload requirements",
            ...
            recommendations.append(
                {
                    ...
                    "category": "ecs_advanced",
                    "resource": f"Cluster: {cluster_name}",
                    "issue": "Cluster has registered container instances that may include ECS Anywhere nodes requiring additional security",
                    "recommendation": "Ensure ECS Anywhere instances follow security best practices for hybrid deployments",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": f"aws ecs describe-container-instances --cluster {cluster_name}",
            ...
            recommendations.append(
                {
                    ...
                    "category": "network_security",
                    "resource": f"VPC: {vpc_id}",
                    "issue": "VPC may lack endpoints for AWS services, potentially routing traffic through internet gateway",
                    "recommendation": "Configure VPC endpoints for ECS, ECR, S3, and CloudWatch to keep traffic within AWS network",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": f"aws ec2 create-vpc-endpoint --vpc-id {vpc_id} --service-name com.amazonaws.{region}.ecs",
                    "compliance_frameworks": ["Network Security", "Data Privacy"],
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "category": "network_security",
                    "resource": f"Service: {service_name}",
                    "issue": "Service DNS configuration should be reviewed for security best practices",
                    "recommendation": "Ensure DNS queries are secured and consider using Route 53 Resolver DNS Firewall",
                    "implementation_steps": [
                    ...
                    "aws_cli_example": f"aws route53resolver create-firewall-domain-list --name {cluster_name}-dns-firewall",
            ...
            recommendations.append(
                {
                    ...
                    "category": "network_security",
                    "resource": f"Service: {service_name}",
                    "issue": f"Service uses {len(security_groups)} security groups which may create complex rule interactions",
                    "recommendation": "Review security group rules for conflicts and implement principle of least privilege",
                    "implementation_steps": [
                    ...
                    recommendations.append(
                        {
                            ...
                            "recommendation": "Ensure EFS file system has encryption at rest enabled with customer-managed KMS key",
                            "implementation_steps": [
                            ...
                            "aws_cli_example": f"aws efs describe-file-systems --file-system-id {file_system_id}",
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": "storage_security",
                            "resource": f"EFS Volume: {volume_name}",
                            "issue": "EFS volume lacks encryption in transit, exposing data during network transmission",
                            "recommendation": "Enable EFS encryption in transit to protect data during network transmission",
                            "implementation_steps": [
                            ...
                            "aws_cli_example": '# Update task definition: "transitEncryption": "ENABLED"',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "storage_security",
                        "resource": f"EFS Volume: {volume_name}",
                        "issue": "EFS file system should have backup and lifecycle policies for data protection",
                        "recommendation": "Configure automatic backups and lifecycle management for EFS file system",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": f"aws efs put-backup-policy --file-system-id {file_system_id} --backup-policy Status=ENABLED",
                    ...
                    recommendations.append(
                        {
                            ...
                            "issue": "EFS access point configuration should be reviewed for proper access controls",
                            "recommendation": "Ensure EFS access point has proper POSIX permissions and path restrictions",
                            "implementation_steps": [
                            ...
                            "aws_cli_example": f"aws efs describe-access-points --access-point-id {access_point_id}",
                ...
                recommendations.append(
                    {
                        ...
                        "category": "storage_security",
                        "resource": f"FSx Volume: {volume_name}",
                        "issue": "FSx file system security configuration should be verified for encryption and access controls",
                        "recommendation": "Ensure FSx file system has proper encryption, backup, and access control configuration",
                        "implementation_steps": [
                ...
                recommendations.append(
                    {
                        ...
                        "category": "storage_security",
                        "resource": f"FSx Volume: {volume_name}",
                        "issue": "FSx file system should have comprehensive backup and versioning strategy",
                        "recommendation": "Implement FSx backup policies and file versioning for data protection",
                        "implementation_steps": [
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": "storage_security",
                            "resource": f"Container: {container_name}",
                            "issue": "Tmpfs mount lacks noexec option, potentially allowing execution of malicious code",
                            "recommendation": "Add noexec, nosuid, and nodev options to tmpfs mounts for enhanced security",
                            "implementation_steps": [
                            ...
                            "aws_cli_example": 'tmpfs: [{"containerPath": "/tmp", "size": 100, "mountOptions": ["noexec", "nosuid", "nodev"]}]',
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": "storage_security",
                            "resource": f"Container: {container_name}",
                            "issue": f"Large tmpfs allocation ({tmpfs_size}MB) may impact system resources",
                            "recommendation": "Review tmpfs size allocation to prevent resource exhaustion attacks",
                            "implementation_steps": [
                ...
                recommendations.append(
                    {
                        ...
                        "category": "storage_security",
                        "resource": f"Container: {container_name}",
                        "issue": f"Container uses {shared_memory_size} bytes of shared memory which should be secured",
                        "recommendation": "Ensure shared memory usage is necessary and properly secured against information disclosure",
                        "implementation_steps": [
                    ...
                    recommendations.append(
                        {
                            ...
                            "category": "storage_security",
                            "resource": f"Container: {container_name}",
                            "issue": f"Large shared memory allocation ({shared_memory_size} bytes) may pose security risks",
                            "recommendation": "Validate shared memory size requirements and implement appropriate limits",
                            "implementation_steps": [
            ...
            recommendations.append(
                {
                    ...
                    "resource": f"Service: {service_name}",
                    "issue": "Envoy proxy configuration should be reviewed for security best practices",
                ...
                recommendations.append(
                    {
                        ...
                        "category": "envoy_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Envoy proxy lacks proper logging configuration for security monitoring",
                        "recommendation": "Configure Envoy access logging for security audit and monitoring",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'proxyConfiguration: {"properties": [{"name": "ENVOY_LOG_LEVEL", "value": "info"}]}',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "envoy_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Envoy admin interface is enabled, potentially exposing sensitive proxy configuration",
                        "recommendation": "Disable Envoy admin interface in production or secure it with proper authentication",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'proxyConfiguration: {"properties": [{"name": "ENVOY_ADMIN_ACCESS_ENABLE", "value": "false"}]}',
                ...
                recommendations.append(
                    {
                        ...
                        "category": "envoy_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Envoy proxy lacks mutual TLS configuration for secure service-to-service communication",
                        "recommendation": "Enable mutual TLS in Envoy proxy for enhanced authentication and encryption",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": 'proxyConfiguration: {"properties": [{"name": "ENVOY_MTLS_ENABLED", "value": "true"}]}',
                        "compliance_frameworks": ["Zero Trust Architecture", "mTLS Best Practices"],
                    }
                )
                ...
                recommendations.append(
                    {
                        ...
                        "category": "envoy_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Envoy proxy lacks circuit breaker configuration for resilience against cascading failures",
                        "recommendation": "Configure circuit breaker settings to protect against service overload",
                        "implementation_steps": [
                        ...
                        "aws_cli_example": "# Configure circuit breaker in App Mesh virtual service",
                ...
                recommendations.append(
                    {
                        ...
                        "category": "envoy_security",
                        "resource": f"Service: {service_name}",
                        "issue": "Envoy proxy lacks rate limiting configuration for DDoS protection",
                        "recommendation": "Implement rate limiting in Envoy proxy to protect against abuse and DDoS attacks",
                        "implementation_steps": [
            ...
            recommendations.append(
                {
                    ...
                    "category": "envoy_security",
                    "resource": f"Service: {service_name}",
                    "issue": "Envoy proxy should be configured to add security headers to responses",
                    "recommendation": "Configure Envoy to add security headers for enhanced web application security",
                    "implementation_steps": [
    ...
    def _generate_why_important(self, title: str, category: str, severity: str) -> str:
        ...
        return importance_map.get(
            title,
            f"This {category} security configuration helps maintain a secure ECS environment and follows AWS security best practices.",
        )
    ...
    def _generate_security_impact(self, title: str, category: str, severity: str) -> str:
        ...
        return impact_map.get(
            title,
            f"This {severity.lower()} severity {category} issue could impact your security posture and compliance requirements.",
        )
    ...
    def _generate_documentation_url(self, title: str, category: str) -> str:
        ...
        doc_map = {
            ...
            "Configure Seccomp Security Profile": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definition_linuxparameters",
            "Configure AppArmor Security Profile": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definition_linuxparameters",
            "Enable No New Privileges Flag": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definition_linuxparameters",
            "Configure Linux Security Parameters": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definition_linuxparameters",
            # PHASE 1: ECR Integration
            "Enable ECR Vulnerability Scanning": "https://docs.aws.amazon.com/AmazonECR/latest/userguide/image-scanning.html",
            "Address Critical Vulnerabilities in Container Image": "https://docs.aws.amazon.com/AmazonECR/latest/userguide/image-scanning.html",
            "Address High Severity Vulnerabilities in Container Image": "https://docs.aws.amazon.com/AmazonECR/latest/userguide/image-scanning.html",
            "Use Amazon ECR for Container Images": "https://docs.aws.amazon.com/AmazonECR/latest/userguide/what-is-ecr.html",
            # PHASE 1: Service Mesh
            "Configure Service Connect Namespace": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html",
            "Enable TLS for Service Connect Communication": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html",
            "Consider ECS Service Connect for Service Mesh": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service-connect.html",
            "Enable TLS in App Mesh Configuration": "https://docs.aws.amazon.com/app-mesh/latest/userguide/tls.html",
            # PHASE 1: Advanced Image Security
            "Implement Container Image Signing": "https://docs.aws.amazon.com/signer/latest/developerguide/Welcome.html",
            "Implement Image Provenance Tracking": "https://docs.aws.amazon.com/signer/latest/developerguide/Welcome.html",
            "Verify Secure Multi-Stage Build Practices": "https://docs.docker.com/develop/dev-best-practices/",
            "Use Minimal Base Images": "https://docs.docker.com/develop/dev-best-practices/",
            "Use Immutable Image Tags with Digest": "https://docs.aws.amazon.com/AmazonECR/latest/userguide/image-tag-mutability.html",
            # PHASE 2: Advanced ECS Features
            "Configure KMS Encryption for ECS Exec": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-exec.html",
            "Enable ECS Exec Session Logging": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-exec.html",
            "Enable Deployment Circuit Breaker for Blue/Green": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/service_definition_parameters.html",
            "Review Spot Instance Security Implications": "https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-spot-instances.html",
            "Verify ECS Anywhere Security Configuration": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-anywhere.html",
            # PHASE 2: Advanced Network Security
            "Configure VPC Endpoints for AWS Services": "https://docs.aws.amazon.com/vpc/latest/privatelink/vpc-endpoints.html",
            "Review DNS Security Configuration": "https://docs.aws.amazon.com/Route53/latest/DeveloperGuide/resolver-dns-firewall.html",
            "Review Multiple Security Groups Configuration": "https://docs.aws.amazon.com/vpc/latest/userguide/VPC_SecurityGroups.html",
            # PHASE 2: Advanced Storage Security
            "Verify EFS Encryption at Rest": "https://docs.aws.amazon.com/efs/latest/ug/encryption.html",
            "Configure EFS Backup and Lifecycle Policies": "https://docs.aws.amazon.com/efs/latest/ug/awsbackup.html",
            "Verify FSx Security Configuration": "https://docs.aws.amazon.com/fsx/latest/WindowsGuide/encryption.html",
            "Configure Secure Tmpfs Mount Options": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definition_linuxparameters",
            "Review Shared Memory Security Configuration": "https://docs.aws.amazon.com/AmazonECS/latest/developerguide/task_definition_parameters.html#container_definition_linuxparameters",
            # PHASE 2: Envoy Proxy Security
            "Configure Envoy Access Logging": "https://docs.aws.amazon.com/app-mesh/latest/userguide/envoy.html",
            "Secure Envoy Admin Interface": "https://docs.aws.amazon.com/app-mesh/latest/userguide/envoy.html",
            "Configure Envoy Circuit Breaker": "https://docs.aws.amazon.com/app-mesh/latest/userguide/circuit-breakers.html",
        }
...
class SecurityReportFormatter:
    ...
    def _generate_resource_breakdown(self, categorized: Dict[str, Any]) -> str:
        ...
        for resource, issues, high_count, medium_count, low_count, _ in resource_priority:
            ...
            section.append(
                f"### {priority_indicator} `{resource}` ({total_issues} issue{'s' if total_issues != 1 else ''})"
            )
    ...
    def _generate_aws_cli_commands(self, title: str, resource: str) -> List[str]:
        ...
        commands_map = {
            "Enable Container Insights": [
                "aws ecs modify-cluster --cluster CLUSTER_NAME --settings name=containerInsights,value=enabled"
            ],
            "Configure Container Health Check": [
...
async def _list_clusters(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "instruction": "Present the cluster list and region options to the user. Wait for their choice. Do not automatically select any cluster.",
            "assessment": f"I found {len(clusters)} ECS clusters in your **{region}** region. Which cluster would you like me to analyze for security recommendations?\n\n**🌍 REGION OPTIONS:**\n• Current region: {region}\n• To check other regions: us-west-2, eu-west-1, ap-southeast-1, ap-northeast-1, etc.\n• Just ask: 'Show me clusters in [region-name]'\n\n**⚠️ IMPORTANT**: I need you to choose a specific cluster name. I cannot automatically select one for you.",
            "clusters": clusters,
            "total_clusters": len(clusters),
            "cluster_selection": {
                "description": "⚠️ STOP: User must choose ONE specific cluster name from the list above. Do NOT automatically select or analyze any cluster without explicit user choice.",
                "region_note": f"🌍 Currently showing {region} region. User can request other regions by asking 'Show me clusters in [region-name]' or specifying 'region' parameter.",
                "available_actions": [
                ...
                "user_choice_required": "⚠️ WAIT: Present this list to the user and ask them to choose a cluster name OR request a different region",
            ...
            "guidance": {
                ...
                "region_selection": f"🌍 **Region Options**: Currently showing {region}. To check other regions, use the 'region' parameter with values like: us-east-1, us-west-2, eu-west-1, ap-southeast-1, ap-northeast-1, etc.",
                "next_steps": [
                    "1. **Present Options**: Show the user this cluster list AND region options",
                    "2. **Wait for Choice**: Let user choose a cluster name OR request different region",
                    "3. **Region Choice**: If user wants different region, ask 'Show me clusters in [region]'",
                    "4. **Cluster Choice**: If user chooses cluster, use exact name from 'name' field",
                ...
                "other_regions": {
                    "description": "🌍 Want to check clusters in other regions? Use these examples:",
                    "examples": {
                        "us_east_1": {
                            "action": "list_clusters",
                            "parameters": {"region": "us-east-1"},
        ...
        return {
            ...
            "next_steps": [
                ...
                for step in [
                    ...
                    f"Review and address {severity_counts['medium']} medium and {severity_counts['low']} low priority items when possible"
                    if severity_counts["medium"] > 0 or severity_counts["low"] > 0
                    else "Consider implementing additional security monitoring for ongoing protection",
                ]
                if step is not None
            ],
            "quick_actions": {
...
async def _get_security_recommendations(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not cluster_name:
        raise ValueError(
            "cluster_name is required for getting recommendations. Use action='list_clusters' to see available clusters, then specify cluster_name in parameters."
        )
...
async def _check_compliance_status(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not cluster_name:
        raise ValueError(
            "cluster_name is required for compliance check. Use action='list_clusters' to see available clusters, then specify cluster_name in parameters."
        )
