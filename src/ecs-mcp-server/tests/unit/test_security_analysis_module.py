...
async def test_tool_execution_list_clusters(mock_format, mock_list_clusters, mock_get_region):
    ...
    result = await registered_tool(region="us-east-1", cluster_names=None)
...
async def test_tool_execution_collect_configuration(
    ...
    result = await registered_tool(region="us-east-1", cluster_names=["test-cluster"])
...
async def test_tool_execution_multiple_clusters(
    ...
    result = await registered_tool(region="us-east-1", cluster_names=["cluster-1", "cluster-2"])
